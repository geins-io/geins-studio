...
- **Entity URLs**: Use `useEntityUrl()` — `getEntityUrl(id)` for current context, `entityDetailHref(key, id)` (`#shared/utils/entities`) for any entity by registry key. Prefer over hardcoded routes.
