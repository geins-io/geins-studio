...
- NEVER pass an already-translated string as `entityName` to an entity-action key. Entity keys (`add_entity`, `no_entity`, `select_entity`, `no_entity_found`, `all_entity`, …) interpolate via `@.lower:{entityName}`, so `entityName` MUST be the raw key name and the **pluralizer goes last**: `t('no_entity', { entityName: 'input' }, 2)` → "No inputs", `t('add_entity', { entityName: 'group' })` → "Add group". NEVER `t('no_entity', { entityName: t('input', 2) })` — the link can't resolve a pre-translated value and it double-translates. The outer count drives the linked plural form (verified: `…,2` → "Inputs"), so omit the count for singular. **Prefer these entity-based keys over bespoke per-entity strings** to keep the locale files small — reuse `add_entity`/`no_entity`/etc. before adding a new key
...
- **Entity keys (i18n key)**: Comes from `EntityKey`/`ENTITIES` (`#shared/utils/entities`) — the single source. Pass an explicit typed literal (`const entityName: EntityKey = 'workflow'`) to `useEntityEdit`/`usePageError`, NEVER a bare untyped string. `useEntityUrl().getEntityName()` derives from the route folder and is for **URL building** only — it equals the i18n key just when folder == key (breaks under plural folders like `workflows`). Ref: [docs/concepts/entities.md](docs/concepts/entities.md).
...
- **Entity registry is the single source of truth.** Every _domain_ entity (endpoint + repo + route) is declared once in `shared/utils/entities.ts` → `ENTITIES` (the key IS the i18n key; descriptor holds `endpoint` + optional `route`). `EntityKey` is derived from it. Each entry is self-describing — `defineEntities` stamps it with its own `key`, so passing `ENTITIES.x` carries endpoint + route + key. Domain repos read their endpoint by passing the entry: `repo.entityFor(ENTITIES.key, fetch)` (≡ `entityRepo(ENTITIES.key.endpoint, fetch, ENTITIES.key.key)`) — NEVER re-declare an endpoint literal that already lives in the registry. Use lower-level `repo.entity(endpoint, fetch, 'key')` only for sub-entities on a scoped/non-registry endpoint (e.g. company-scoped `buyer`). Label-only entity-name keys (`name`, `currency`, …) are NOT domain entities — they stay as plain i18n keys (raw string literals at the call site), not the registry; the `entities.test.ts` en↔sv key-parity check guards them. Ref: [docs/concepts/entities.md](docs/concepts/entities.md).
