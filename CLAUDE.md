...
- **Entity URLs**: Build from the registry helpers in `#shared/utils/entities` — `entityListHref(key)` (the collection index), `entityNewHref(key)`, `entityDetailHref(key, id)`, `entityChildPattern(key)` (nav). Pure functions off `ENTITIES[key].route`; NEVER hardcode `/domain/entity/...`. The create segment is the single `NEW_ENTITY_URL_SEGMENT` constant (white-label knob — set `'add'`/`'create'` in one place). `useEntityEdit` exposes `newEntityUrl`/`entityListUrl` for the current entity (derived from its entry), so pages need the helpers mainly on list/index pages + cross-entity links. (There is no `useEntityUrl` composable — removed.)
- **Entity identity**: `useEntityEdit`/`usePageError` take the registry entry `entity: ENTITIES.x` (`#shared/utils/entities`) — the single source; they derive the i18n key (`entityName`) from `entity.key`. NEVER pass a bare string. For display, use the `entityName` string returned by `useEntityEdit` (or `ENTITIES.x.key`); for label-only interpolation use raw key literals. Ref: [docs/concepts/entities.md](docs/concepts/entities.md).
