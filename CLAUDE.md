...
- **Entity registry is the single source of truth.** Every _domain_ entity (endpoint + repo + route) is declared once in `shared/utils/entities.ts` → `ENTITIES` (the key IS the i18n key; descriptor holds `endpoint` + optional `route`). `EntityKey` is derived from it. Domain repos read their endpoint from the registry via `repo.entityFor('key', fetch)` (≡ `entityRepo(ENTITIES.key.endpoint, fetch, 'key')`) — NEVER re-declare an endpoint literal that already lives in the registry. Use lower-level `repo.entity(endpoint, fetch, 'key')` only for sub-entities on a scoped/non-registry endpoint (e.g. company-scoped `buyer`). Label-only entity-name keys (`name`, `currency`, …) are NOT domain entities — they stay as plain i18n keys (raw string literals at the call site), not the registry; the `entities.test.ts` en↔sv key-parity check guards them. Ref: [docs/concepts/entities.md](docs/concepts/entities.md).
