...
The `useAssetCapabilities` composable reports which Assets Library features are available for the configured backend. The shipped UI gates on it so controls the real **Geins.Media phase 1** API can't fulfil yet — rename, tags, channels, folder move, replace, tag autocomplete — disable cleanly instead of erroring, while the Supabase **mock** keeps everything on.

:::tip WHY
Studio built the full v0 UI against the mock. The real backend's phase 1 serves **browse + upload, plus description/alt-text edit (`PATCH`) and delete (`DELETE` + restore)**; the rest arrives in phase 2. Rather than delete that UI, we gate it per field — see the _Phase 8 — Real API alignment_ milestone.
:::
...
| `canRenameAsset`            | `boolean`       | Editing an asset's `name`.                                        |
