...
Everything else the Phase 8 prep added turned out to be the real contract and simply stayed: `path`/`folderPath`/`depth`, the asset `etag` and the client's `If-Match` + `412` handling, per-locale `description` in `localizations`, `POST /media/assets/query` with `folderIds` (though `all: true` turned out to override filters — it's now sent only for the unfiltered view, and the list fetches one 1000-asset page, the schema's cap), the three folder scopes, the ticket upload client, `relocate`, trash + restore, and the `AssetLink` shape.
