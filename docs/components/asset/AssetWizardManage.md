...
**Capability gating.** `name` + folder + description + alt text all persist via the ticket claim itself (applied when the upload completes, no follow-up write) — all editable. **Tags + channels** have no phase-1 upload route, so they render in a disabled `<fieldset>` with a "not saved yet" hint (`useAssetCapabilities` → `canEditTags` / `canEditChannels`); values are still held in wizard state. Tag autocomplete only fetches when `tagAutocomplete` is on. The bulk pane ([`AssetWizardBulkPane`](/components/asset/AssetWizardBulkPane)) gates the same two fields. (`cutover: REVISIT@phase2`.)
