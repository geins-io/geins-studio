...
const readPosition = (node: WorkflowNode & WithUi, index: number) =>
  node.ui?.position
    ?? node.position
    ?? { x: DEFAULT_X + index * DEFAULT_X_STEP, y: DEFAULT_Y }
...
const readEndpoints = (c: Record<string, unknown>): { source?: string, target?: string } => ({
  source: (c.sourceNodeId ?? c.source ?? c.from ?? c.fromNodeId) as string | undefined,
  target: (c.targetNodeId ?? c.target ?? c.to ?? c.toNodeId) as string | undefined,
})

const toCanvasEdge = (c: WorkflowNodeConnection & WithUi, index: number): Edge => {
  const { source, target } = readEndpoints(c as unknown as Record<string, unknown>)
  ...
  const sourceHandle = c.type === 'conditional' && c.label ? c.label : undefined
  return {
...
export const useWorkflowCanvas = (): WorkflowCanvasReturnType => {
  ...
  const toApi: WorkflowCanvasReturnType['toApi'] = ({ nodes, edges }) => ({
    ...
    connections: edges.filter(e => !(e.data as { synthetic?: boolean } | undefined)?.synthetic).map((e) => {
      const storedType = (e.data as { type?: ConnectionType } | undefined)?.type
      const existingUi = ((e.data as { ui?: NodeUi } | undefined)?.ui) ?? {}
      const type: ConnectionType = storedType
        ?? (e.sourceHandle === 'true' || e.sourceHandle === 'false' ? 'conditional' : 'sequential')
      const label = typeof e.label === 'string' && e.label
        ? e.label
        : type === 'conditional' ? e.sourceHandle ?? undefined : undefined
      return {
