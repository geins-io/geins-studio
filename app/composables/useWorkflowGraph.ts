...
export type WorkflowNodeLike = {
  ...
  conditions?: Array<{ label: string, condition?: string, description?: string }> | null
  defaultLabel?: string
  loop?: AnyRecord | null
...
const LEVEL_X = 320
const ROW_Y = 140
const KNOWN_VUEFLOW_TYPES = new Set(['trigger', 'action', 'condition', 'loop', 'delay'])
...
function classifyEdgeKind(conn: WorkflowConnectionLike): FlowEdge['data']['kind'] {
...
export function useWorkflowGraph(source: MaybeRefOrGetter<WorkflowLike | null | undefined>) {
  const nodes = computed<FlowNode[]>(() => {
    ...
    return rawNodes.map(n => toFlowNode(n, positions.get(n.id) ?? { x: 0, y: 0 }))
  })
