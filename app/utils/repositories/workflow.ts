import type {
  ...
  WorkflowEntityCreate as WorkflowCreate,
  WorkflowEntityUpdate as WorkflowUpdate,
  WorkflowApiOptions,
  WorkflowEntityValidationResult as WorkflowValidationResult,
  WorkflowEntityExecution as WorkflowExecution,
  WorkflowExecutionApiOptions,
  WorkflowEntityExecutionStartRequest as WorkflowExecutionStartRequest,
  WorkflowEntityExecutionLog as WorkflowExecutionLog,
  WorkflowEntityExecutionConcurrency as WorkflowExecutionConcurrency,
  WorkflowEntityMetrics as WorkflowRepoMetrics,
  WorkflowEntityAggregateMetrics as WorkflowAggregateMetrics,
  WorkflowEntityErrorSummary as WorkflowErrorSummary,
  WorkflowEntityVersion as WorkflowVersion,
  WorkflowEntityVersionComparison as WorkflowVersionComparison,
  WorkflowEntityVariable as WorkflowRepoVariable,
  WorkflowEntityVariableCreate as WorkflowVariableCreate,
...
export function workflowRepo(fetch: $Fetch<unknown, NitroFetchRequest>) {
  return {
     ...
     */
    metrics: {
      async get(workflowId: string): Promise<WorkflowRepoMetrics> {
        return await fetch<WorkflowRepoMetrics>(
          `${WORKFLOW_DEFINITION_ENDPOINT}/${workflowId}/metric`,
        );
      },

      async listAll(): Promise<WorkflowRepoMetrics[]> {
        return await fetch<WorkflowRepoMetrics[]>(METRIC_ENDPOINT);
      },
     ...
     */
    variable: {
      async list(): Promise<WorkflowRepoVariable[]> {
        return await fetch<WorkflowRepoVariable[]>(VARIABLE_ENDPOINT);
      },

      async get(key: string): Promise<WorkflowRepoVariable> {
        return await fetch<WorkflowRepoVariable>(`${VARIABLE_ENDPOINT}/${key}`);
      },

      async save(data: WorkflowVariableCreate): Promise<WorkflowRepoVariable> {
        return await fetch<WorkflowRepoVariable>(VARIABLE_ENDPOINT, {
          method: 'POST',
          body: data,
        });
    ...
    editor: {
      ...
      async listActions(): Promise<WorkflowEditorAction[]> {
        return await fetch<WorkflowEditorAction[]>(ACTION_ENDPOINT);
      },
    },
  };
}
