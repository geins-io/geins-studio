...
const findItem = (value: number | undefined) => {
  if (value === undefined) return undefined;
  return dataSet.value.find(item => item.value === String(value));
};

const choice = ref<{ label: string; value: string } | undefined>(findItem(model.value));
...
watch(choice, (newChoice) => {
  const newValue = newChoice ? Number(newChoice.value) : undefined;
...
watch([model, () => props.brands], ([newModelValue]) => {
  if (choice.value?.value !== String(newModelValue)) {
