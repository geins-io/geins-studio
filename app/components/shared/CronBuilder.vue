...
function parseCronToState(cron: string) {
  ...
  const [_sec, min, hour, dom, _month, dow] = parts as [
    string,
    string,
    string,
    string,
  ...
  if (dow !== '*') selectedDays.value = dow.split(',').map(Number);
  if (dom !== '*' && dom !== 'L')
    selectedDayOfMonth.value = Number.parseInt(dom) || 1;
  ...
  <div class="space-y-3">
    ...
    <div v-if="frequency === 'hourly'" class="flex items-center gap-2">
      ...
      <Select
        ...
        @update:model-value="selectedMinute = Number($event)"
      >
    ...
    <div v-if="frequency === 'daily'" class="flex items-center gap-2">
      ...
      <Select
        ...
        @update:model-value="selectedHour = Number($event)"
      >
        <SelectTrigger class="w-20">
          <SelectValue />
        </SelectTrigger>
    ...
    <div v-if="frequency === 'monthly'" class="space-y-2">
      <div class="flex items-center gap-2">
        ...
        <Select
          ...
          @update:model-value="
            ...
            selectedDayOfMonth = Number($event);
          "
        >
          <SelectTrigger class="w-20">
            <SelectValue />
      ...
      >
        ...
        Day {{ selectedDayOfMonth }} doesn't exist in all months — the workflow
        will skip those months.
      </p>
      <div class="flex items-center gap-2">
        <span class="text-muted-foreground text-xs">at</span>
        <Select
