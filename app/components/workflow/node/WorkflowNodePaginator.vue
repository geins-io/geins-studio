...
const IconComponent = computed(
  () => resolveIcon(props.data.icon) || resolveIcon('BookOpen'),
);
...
<template>
  <div
    ...
    :class="
      selected
        ? 'border-indigo-500 ring-[6px] ring-indigo-500/20'
        : 'border-indigo-500/50'
    "
  >
    <!-- Input handle -->
    <WorkflowHandleInput
      handle-class="!border-background !h-[15px] !w-[15px] !border-2 !bg-indigo-500"
    />

    <div class="flex items-center gap-3">
      <div
        class="flex h-10 w-10 items-center justify-center rounded-lg bg-indigo-500/10 text-indigo-500"
      >
