...
const normalizedType = computed(() =>
  (props.data.triggerType ?? 'onDemand').toLowerCase(),
);
...
const IconComponent = computed(() => {
  switch (normalizedType.value) {
    ...
    default:
      return LucideMousePointerClick;
  }
});
...
<template>
  <div
    ...
    :class="
      selected
        ? 'border-green-500 ring-[6px] ring-green-500/20'
        : 'border-green-500/50'
    "
  >
    <div
      class="flex w-14 shrink-0 items-center justify-center bg-green-500/10 text-green-600 dark:text-green-400"
    >
    ...
    <div class="flex min-w-0 flex-1 flex-col justify-center gap-0.5 py-2 pl-3">
      ...
      <div v-if="eventPattern" class="mt-0.5 flex">
        <span
          class="bg-muted text-muted-foreground inline-block max-w-full truncate rounded-full px-2 py-0.5 font-mono text-[10px] leading-tight"
        >
          {{ eventPattern }}
        </span>
      </div>
