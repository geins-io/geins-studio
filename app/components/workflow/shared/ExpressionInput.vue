...
const mode = ref<'fixed' | 'expression'>(
  hasExpression(props.modelValue) ? 'expression' : props.defaultMode,
);
...
function parseFunctionContext(
  doc: string,
  pos: number,
): { fnName: string; paramIndex: number } | null {
...
const truncatedPreview = computed(() => {
  ...
  return preview.value.length > 120
...
function extractFunctionArgs(
  afterBraces: string,
): { fnName: string; args: string[] } | null {
  const fnMatch = afterBraces.match(/^(\w+)\(/);
  if (!fnMatch || !fnMatch[1]) return null;
...
function walkObjectForJsonPaths(
  ...
  if (Array.isArray(obj)) {
    results.push({
      path: `${prefix}[0]`,
      preview: `first of ${obj.length} items`,
    });
    results.push({ path: `${prefix}[*]`, preview: `all ${obj.length} items` });
    if (obj.length > 0) {
      walkObjectForJsonPaths(
        obj[0],
        `${prefix}[0]`,
        results,
        maxDepth,
...
function getJsonPathCompletions(
  afterBraces: string,
...
function isInsideFunctionParens(
  afterBraces: string,
): { insideFn: true; argText: string } | { insideFn: false } {
  let depth = 0;
  let lastArgStart = -1;
  ...
  if (parenCtx.insideFn) {
    ...
    if (jsonPaths && jsonPaths.length > 0) {
      const filtered = typed
        ? jsonPaths.filter((jp) =>
            jp.path.toLowerCase().includes(typed.toLowerCase()),
          )
        : jsonPaths;
...
<template>
  <div class="group relative">
    ...
    <div v-else>
      ...
      >
        ...
        >
          ...
          <span
            :class="
              i === paramHint.paramIndex
                ? 'text-foreground rounded bg-emerald-500/20 px-1 font-semibold'
    ...
    >
      ...
      <TooltipProvider
        v-if="mode === 'expression' && expressionFunctions.length > 0"
        :delay-duration="300"
      >
        <Tooltip :open="!showFnRef ? undefined : false">
          <Popover v-model:open="showFnRef">
            ...
            <PopoverContent align="end" :side-offset="8" class="w-[360px] p-0">
              <!-- Category tabs -->
              <div
                class="scroll-visible flex gap-1 overflow-x-auto border-b px-2 py-1.5"
              >
                <button
