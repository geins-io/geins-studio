...
const upstreamNodes = computed(() => {
  ...
  while (queue.length) {
    ...
    for (const edge of incomingEdges) {
      ...
      const sourceHandle =
        current === props.nodeId ? (edge.sourceHandle ?? undefined) : undefined;
      const outputFields = resolveOutputFields(
        n.id,
        nodeType,
        data,
        sourceHandle,
...
const fieldExpression = (upstream: UpstreamNode, fieldName: string) =>
  isIteratorContextVar(fieldName)
    ? `{{${fieldName}}}`
    : `{{output.${upstream.id}.${fieldName}}}`;
...
const filteredUpstreamNodes = computed(() => {
  ...
  return upstreamNodes.value
    .map((n) => ({
      ...n,
      outputFields: n.outputFields.filter(
        (f) =>
          f.name.toLowerCase().includes(q) || f.type.toLowerCase().includes(q),
      ),
    }))
    .filter(
      (n) => n.outputFields.length > 0 || n.label.toLowerCase().includes(q),
    );
});
...
const filteredVariables = computed(() => {
  ...
  return workflowVariables.value.filter(
    (v) =>
...
const filteredInputDefs = computed(() => {
  ...
  return workflowInputDefs.value.filter(
...
function upstreamOutputPreview(
  nodeId: string,
  fieldName: string,
): string | null {
        ...
        <div v-if="inputPaneSections.has('incoming')" class="px-3 pb-3">
          <div v-if="filteredUpstreamNodes.length" class="space-y-2">
            <div v-for="upstream in filteredUpstreamNodes" :key="upstream.id">
              <div
                class="mb-1 flex items-center gap-1 text-[10px] font-medium tracking-wider uppercase"
                :class="upstreamLabelClass(upstream.nodeType)"
              >
                <LucideCircle
                  v-if="!upstreamHasOutput(upstream.id)"
                  class="h-2 w-2"
                  :class="upstreamDotClass(upstream.nodeType)"
                />
              ...
              <div v-if="upstream.outputFields.length" class="space-y-0.5">
                ...
                >
                  <Tooltip
                    :disabled="!upstreamOutputPreview(upstream.id, field.name)"
                  >
                    <TooltipTrigger as-child>
                      <button
                        ...
                        "
                        @dragstart="
                          onExpressionDragStart(
                            $event,
                            fieldExpression(upstream, field.name),
                          )
                        "
                      >
                    ...
                    >
                      <pre
                        class="max-h-48 overflow-auto font-mono text-[11px] break-all whitespace-pre-wrap"
                        >{{
                          truncatePreview(
                            upstreamOutputPreview(upstream.id, field.name) ??
                              '',
              ...
              <Tooltip :disabled="!runDataPreview(inp.name)">
                <TooltipTrigger as-child>
                  <button
                    ...
                    :title="
                      runDataPreview(inp.name)
                        ? undefined
                        : `${inp.type}${inp.description ? ' — ' + inp.description : ''}\n{{input.${inp.name}}}`
                    "
                    @click="copyExpression(`{{input.${inp.name}}}`)"
                    @dragstart="
                      onExpressionDragStart($event, `{{input.${inp.name}}}`)
                    "
                  >
                ...
                <TooltipContent side="right" :side-offset="8" class="max-w-xs">
                  <pre
                    class="max-h-48 overflow-auto font-mono text-[11px] break-all whitespace-pre-wrap"
                    >{{ truncatePreview(runDataPreview(inp.name) ?? '') }}</pre
                  >
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
              ...
              <Tooltip :disabled="!runDataPreview(v.key)">
                <TooltipTrigger as-child>
                  <button
                    ...
                    @dragstart="
                      onExpressionDragStart($event, `{{vars.${v.key}}}`)
                    "
                  >
                ...
                <TooltipContent side="right" :side-offset="8" class="max-w-xs">
                  <pre
                    class="max-h-48 overflow-auto font-mono text-[11px] break-all whitespace-pre-wrap"
                    >{{ truncatePreview(runDataPreview(v.key) ?? '') }}</pre
                  >
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
