...
const requiredFields = computed(() => actionInputFields.value.filter(f => f.required))
const optionalFields = computed(() => actionInputFields.value.filter(f => !f.required))
  ...
  <div class="h-full">
    ...
    <template v-else-if="actionInputFields.length">
      ...
      <div v-if="requiredFields.length" class="space-y-4">
        <div v-for="field in requiredFields" :key="field.name" class="space-y-1.5">
          ...
          <div v-else-if="isBoolean(field)" class="flex items-center gap-2">
            ...
            <span class="text-muted-foreground text-xs">
              {{ Boolean(nodeInput[field.name] ?? field.default) ? 'Enabled' : 'Disabled' }}
            </span>
          </div>
          <ExpressionInput
            v-else
            ...
            size="sm"
            :default-mode="fieldToString(field).includes('{{') ? 'expression' : 'fixed'"
            @update:model-value="updateInput(field.name, $event)"
          />
      ...
      <Separator v-if="requiredFields.length && optionalFields.length" class="my-4" />

      <!-- Optional fields in collapsible -->
      <Collapsible v-if="optionalFields.length" v-model:open="optionalOpen">
        <CollapsibleTrigger class="text-muted-foreground hover:text-foreground flex w-full items-center gap-1.5 text-xs font-medium transition-colors">
          <LucideChevronRight class="size-3.5 transition-transform" :class="optionalOpen && 'rotate-90'" />
          Optional ({{ optionalFields.length }})
        </CollapsibleTrigger>
        <CollapsibleContent>
          <div class="mt-3 space-y-4">
            <div v-for="field in optionalFields" :key="field.name" class="space-y-1.5">
              <Label class="text-muted-foreground flex items-center gap-1.5 text-sm">
                {{ prettyLabel(field.name) }}
                <span
                  class="rounded px-1.5 py-0.5 font-mono text-[10px] font-medium"
    ...
    <!-- Empty state -->
    <div v-else-if="!customComponent" class="text-muted-foreground flex flex-col items-center gap-2 py-8 text-center text-sm">
