...
const removeEdgesForSourceHandle = inject<
  (sourceId: string, handleId: string) => void
>('removeEdgesForSourceHandle', () => {});
const renameEdgeSourceHandle = inject<
  (sourceId: string, oldHandle: string, newHandle: string) => void
>('renameEdgeSourceHandle', () => {});
...
function parseExpression(expr: string): BuilderClause | null {
  ...
  if (compMatch) {
    ...
    return {
      field: field.trim(),
      operator: operator as OperatorValue,
      value: val,
    };
  }
...
function serializeClause(clause: BuilderClause): string {
  ...
  if (fn) {
    return fn.template
      .replace('{field}', clause.field)
      .replace('{value}', clause.value);
  }
...
function toggleMode(index: number) {
  ...
  conditionModes.value[index] =
    current === 'builder' ? 'expression' : 'builder';
}
...
function getBuilderClause(index: number): BuilderClause {
  ...
  return (
    parseExpression(cond.condition) ?? { field: '', operator: '==', value: '' }
  );
}
...
function applyTemplate(tmpl: (typeof TEMPLATES)[number]) {
  ...
  const updated = [
    ...conditions.value,
    { label: '', condition: tmpl.condition, description: '' },
  ];
  persistColors([...conditionColors.value, color]);
  updateConditions(updated);
}
...
const conditionColors = computed<number[]>(
  () =>
    (Array.isArray(nodeUi.value.conditionColors)
      ? nodeUi.value.conditionColors
      : []) as number[],
);
  ...
  <div class="space-y-4">
      ...
      <div class="space-y-2">
        <div
          ...
          :class="[
            ...
            touched[i]?.size &&
            Object.keys(validationErrors[i] ?? {}).length > 0
        ...
        >
          ...
          <div class="flex items-center gap-2 px-3 pt-2.5 pb-1">
            ...
            >
              <LucideGripVertical
                class="text-muted-foreground/40 group-hover/card:text-muted-foreground h-3 w-3 transition-colors"
              />
              <span
                class="flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold"
                :class="branchColor(getConditionColor(i)).badge"
              >
                {{ i + 1 }}
              </span>
            </div>
            ...
            <div class="min-w-0 flex-1">
              <Input
                ...
                @update:model-value="
                  updateConditionField(i, 'label', String($event))
                "
              />
              <p
                v-if="getError(i, 'label')"
                class="text-destructive px-1 text-[10px]"
              >
                {{ getError(i, 'label') }}
              </p>
            </div>
            ...
            <!-- Actions -->
            <div
              class="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover/card:opacity-100 focus-within:opacity-100"
            >
              <!-- Mode toggle -->
              <button
                type="button"
          ...
          <div class="px-3 pt-1 pb-3">
            ...
            <div v-if="getMode(i) === 'builder'" class="space-y-1.5">
              ...
              >
                <PopoverTrigger as-child>
                  <button
                    ...
                    @click="
                      activeFieldPopover = activeFieldPopover === i ? null : i
                    "
                  >
                    <LucideVariable
                ...
                <PopoverContent
                  v-if="activeFieldPopover === i"
                  class="w-[min(500px,calc(100vw-2rem))] p-0"
                  align="start"
                >
                  ...
                  <div class="max-h-72 overflow-y-auto p-1">
                    <template v-if="filteredFields.length > 0">
                      <template
                        v-for="[section, fields] in groupedFields"
                        :key="section"
                      >
                        <div
              ...
              <Select
                ...
                @update:model-value="
                  updateBuilderClause(i, { operator: $event as OperatorValue })
                "
              >
              ...
              <div v-if="operatorNeedsValue(getBuilderClause(i).operator)">
                <Input
                  ...
                  @update:model-value="
                    updateBuilderClause(i, { value: String($event) })
                  "
                />
                <p
                  v-if="getError(i, 'value')"
                  class="text-destructive mt-0.5 text-[10px]"
                >
                  {{ getError(i, 'value') }}
                </p>
              </div>
              ...
              <ExpressionInput
                ...
                :class="
                  getError(i, 'condition')
                    ? '[&_textarea]:border-destructive/50'
                    : ''
                "
                @blur="markTouched(i, 'condition')"
                @update:model-value="
                  updateConditionField(i, 'condition', String($event))
                "
              />
              <p
                v-if="getError(i, 'condition')"
                class="text-destructive text-[10px]"
              >
                {{ getError(i, 'condition') }}
              </p>
            ...
            <div class="mt-2">
              <Input
                ...
                @update:model-value="
                  updateConditionField(i, 'description', String($event))
                "
              />
            </div>
          </div>
    ...
    <div class="border-t pt-4">
      <div class="flex items-center gap-2">
        <span
          class="bg-muted-foreground/15 text-muted-foreground flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold"
        >
          *
        </span>
