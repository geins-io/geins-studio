...
const initialMode: DelayMode = props.nodeInput.waitUntil !== undefined && props.nodeInput.waitUntil !== null
  ? 'waitUntil'
  : 'duration'
...
function parseIsoDuration(iso: string): { amount: number, unit: UnitValue } {
  ...
  if (match && match[1] && match[2]) {
    return { amount: Number(match[1]), unit: match[2].toUpperCase() as UnitValue }
  }
...
const parsed = computed(() => parseIsoDuration(String(props.nodeInput.duration ?? 'PT5M')))
...
function validateWaitUntil(val: string): string | null {
  ...
  if (!ISO_DATETIME_RE.test(val)) return 'Must be an ISO 8601 datetime (e.g. 2026-03-06T15:00:00Z) or an expression'
...
<template>
  <div class="space-y-4">
    ...
    <div class="space-y-1.5">
      <label class="text-muted-foreground text-xs font-medium">Delay type</label>
      ...
      <div class="space-y-1.5">
        ...
        <div class="flex gap-2">
          <Input
            ...
            min="1"
            class="flex-1"
            @update:model-value="updateDuration(Number($event) || 1, durationUnit)"
          />
          <Select
            :model-value="durationUnit"
            @update:model-value="updateDuration(durationAmount, $event as UnitValue)"
          >
