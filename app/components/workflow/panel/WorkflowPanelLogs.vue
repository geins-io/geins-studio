...
watch(height, (v) => {
  if (typeof window !== 'undefined')
    window.localStorage.setItem(STORAGE_KEY_HEIGHT, String(Math.round(v)));
});
...
const streamStatus = ref<'idle' | 'connecting' | 'live' | 'done' | 'error'>(
  'idle',
);
...
function statusKind(status: string): 'running' | 'success' | 'error' | 'other' {
  ...
  if (
    ...
    return 'error';
  if (s === 'running' || s === 'started' || s === 'pending' || s === 'active')
    return 'running';
  return 'other';
}
    ...
    typeof raw.error === 'string'
      ...
      : typeof raw.errorMessage === 'string'
        ...
        : typeof raw.message === 'string'
          ...
          : null;
  const input =
    raw.input != null && typeof raw.input === 'object'
      ? (raw.input as Record<string, unknown>)
      : null;
  const output =
    raw.output != null && typeof raw.output === 'object'
...
async function pollOnce(execId: string): Promise<boolean> {
  try {
    const details = (await orchestratorApi.execution.get(
      execId,
    )) as ExecutionDetailsResponse;
    ...
    const inner = (asRecord(detailsRecord.execution) ??
    ...
    for (const nr of nodeResults) {
      if (nr.error && nr.nodeId)
        nodeResultErrors.set(nr.nodeId as string, nr.error as string);
    }

    // Execution-level errors (fallback when per-node error is unavailable)
    const execErrors: string[] = Array.isArray(inner?.errors)
      ? inner.errors
      : [];
    ...
    if (execErrors.length > 0) {
      const failedEvent = events.value.find(
        (e) => statusKind(e.status) === 'error' && !e.error,
      );
      if (failedEvent) {
...
async function popOut() {
  ...
  root.style.cssText =
    'height:100vh; display:flex; flex-direction:column; background:var(--background);';
      ...
      <Teleport :to="popoutContainer ?? undefined" :disabled="!isPoppedOut">
        <div class="bg-background flex h-full flex-col overflow-hidden">
          <slot>
            <!-- Live stream toolbar -->
            <div
              class="bg-muted/30 flex h-9 shrink-0 items-center justify-between border-b px-3 text-xs"
            >
              <div class="flex items-center gap-2">
            ...
            <div class="min-h-0 flex-1 overflow-auto">
              <div
                ...
                class="text-muted-foreground flex h-full items-center justify-center p-6 text-xs"
              >
                {{
                  props.executionId
              ...
              <table v-else class="w-full text-xs">
                ...
                <tbody>
                  <template
                    v-for="event in eventsSorted"
                    :key="`${event.seq}:${event.nodeId}`"
                  >
                    <tr
                      ...
                      :class="{
                        '!border-b-0':
                          (event.error &&
                            statusKind(event.status) === 'error') ||
                          expandedNodeId === event.nodeId,
                        'bg-muted/20': expandedNodeId === event.nodeId,
                      }"
                      @click="
                        toggleExpand(event.nodeId);
                        emit('select:node', event.nodeId);
                      "
                    >
                      <td class="text-muted-foreground px-3 py-1 font-mono">
                        {{ event.seq >= 0 ? event.seq : '–' }}
                      </td>
                      <td class="px-3 py-1 font-mono">
                        <span class="flex items-center gap-1">
                          <LucideChevronRight
                            v-if="hasRunData(event)"
                            class="text-muted-foreground h-3 w-3 shrink-0 transition-transform"
                            :class="{
                              'rotate-90': expandedNodeId === event.nodeId,
                            }"
                          />
                        ...
                        <span
                          ...
                          :class="{
                            'text-green-600 dark:text-green-500':
                              statusKind(event.status) === 'success',
                            'text-red-600 dark:text-red-500':
                              statusKind(event.status) === 'error',
                            'text-blue-600 dark:text-blue-400':
                              statusKind(event.status) === 'running',
                            'text-muted-foreground':
                              statusKind(event.status) === 'other',
                          }"
                        >
                          <span
                    ...
                    <tr
                      v-if="event.error && statusKind(event.status) === 'error'"
                      class="border-b last:border-b-0"
                      :class="{
                        '!border-b-0': expandedNodeId === event.nodeId,
                      }"
                    >
                      <td />
                    ...
                    <tr
                      v-if="
                        expandedNodeId === event.nodeId && hasRunData(event)
                      "
                      class="border-b last:border-b-0"
                    >
                      <td />
                      <td colspan="4" class="px-3 pt-0.5 pb-2">
                        <div
                          class="bg-muted/30 flex flex-col overflow-hidden rounded border"
                        >
                          <div
                            class="flex items-center gap-0.5 border-b px-2 py-1"
                          >
                            <button
                          ...
                          <div class="h-48">
                            <JsonCodeEditor
                              v-if="
                                expandedTab === 'input' && event.input != null
                              "
                            ...
                            <JsonCodeEditor
                              v-else-if="
                                expandedTab === 'output' && event.output != null
                              "
              ...
              <tbody>
                ...
                >
                  <tr
                    ...
                    :class="{
                      '!border-b-0':
                        (event.error && statusKind(event.status) === 'error') ||
                        expandedNodeId === event.nodeId,
                      'bg-muted/20': expandedNodeId === event.nodeId,
                    }"
                  ...
                  <tr
                    v-if="event.error && statusKind(event.status) === 'error'"
                    class="border-b last:border-b-0"
                    :class="{ '!border-b-0': expandedNodeId === event.nodeId }"
                  >
                    <td />
                    <td colspan="4" class="px-3 pt-0.5 pb-2">
                      <div
                  ...
                  <tr
                    v-if="expandedNodeId === event.nodeId && hasRunData(event)"
                    class="border-b last:border-b-0"
                  >
                    <td />
                    <td colspan="4" class="px-3 pt-0.5 pb-2">
