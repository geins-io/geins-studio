...
type AddressContentField = (typeof addressContentFields)[number];

const hasAddressContentChanged = (
  snapshot: QuotationAddress | null,
  live: QuotationAddress,
): boolean => {
  if (!snapshot) return false;
  return addressContentFields.some(
    (field: AddressContentField) => snapshot[field] !== live[field],
  );
};

/**
 * Syncs address display refs and entityDataUpdate when the live company addresses
 * differ from the saved quotation snapshots. Only runs in draft mode.
 * Called after setOriginalSavedData() so that detected differences surface as
 * unsaved changes — the next save will re-snapshot the fresh address data.
 */
const syncCompanySnapshots = () => {
  if (createMode.value || sentMode.value) return;
  if (!selectedCompany.value?.addresses || !entityDataUpdate.value) return;

  const liveAddresses = selectedCompany.value.addresses;
  let updated = false;

  // Sync billing address
  if (selectedBillingAddressId.value) {
    const liveAddr = liveAddresses.find(
      (a) => a._id === selectedBillingAddressId.value,
    );
    if (liveAddr) {
      const liveBilling = toQuotationAddress(liveAddr);
      if (hasAddressContentChanged(billingAddress.value, liveBilling)) {
  ...
  if (selectedShippingAddressId.value) {
    ...
    if (liveAddr) {
      const liveShipping = toQuotationAddress(liveAddr);
      if (hasAddressContentChanged(shippingAddress.value, liveShipping)) {
  ...
  if (updated) {
    ...
    } as typeof entityDataUpdate.value;
  }
};
...
if (!createMode.value) {
  ...
  onMounted(async () => {
    ...
    // Sync address snapshots against live company data in draft mode.
    // Called after setOriginalSavedData() so detected address changes
    // surface as unsaved changes and are persisted on the next save.
